...
pList = []
Dmaa_all = arcpy.CreateFeatureclass_management(outPath,'石渠县河流管理范围线.shp', 'POLYLINE')
arcpy.DefineProjection_management(Dmaa_all, coor_system='4542')
arcpy.AddField_management(Dmaa_all, '河流名称', 'TEXT')
...
cs = arcpy.da.InsertCursor(Dmaa_all, ['SHAPE@', '河流名称', '省', '市（州）', '区（县）', '乡镇'])
for cn in pList:
    cs.insertRow(cn)