...
import os
import pandas as pd
path = r'D:\2021年项目\0713江北数据库'
arcpy.env.workspace = path
walk = arcpy.da.Walk(r'D:\2021年项目\0713江北数据库\江北四图层数据库（安装实体桩前）', datatype="FeatureClass", type='Point')
for i, v, m in walk:
    for s in m:
        try:
            if 'DMAP' in s:
                dmap =  os.path.join(i,s)
                dmap_shp = path + '\\' + 'dmap' + '\\' + '{}.shp'.format(dmap.encode('utf-8').split('.mdb')[0].split('\\')[-1])
                dmap_sorted = path + '\\' + 'dmap' + '\\' + '{}_sorted.shp'.format(dmap.encode('utf-8').split('.mdb')[0].split('\\')[-1])
                ...
                arcpy.AddField_management(dmap_shp, '备注'.decode('utf-8'), 'TEXT')
                arcpy.AddField_management(dmap_shp, 'px', 'TEXT')
