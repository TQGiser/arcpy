...
wb = load_workbook(r'D:\2021年项目\0908州河表格\ys.xlsx'.decode('utf-8'))
ws = wb.worksheets[0]
for row in ws.rows:
    x = row[6].value
    y = row[5].value
    e = row[3].value
    if e<100.5:
        row[3].value = XY2LatLon(x,y,99)[1]
        row[4].value = XY2LatLon(x, y,99)[0]
    elif e>100.5:
        row[3].value = XY2LatLon(x, y, 102)[1]
        row[4].value = XY2LatLon(x, y, 102)[0]
    print x,y
wb.save(r'D:\2021年项目\0908州河表格\1.xlsx'.decode('utf-8'))