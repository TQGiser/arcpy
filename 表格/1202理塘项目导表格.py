...
for mdb in mdbs:
    ...
    spatial_ref = arcpy.Describe(mdb).children[0].spatialReference
    ...
    with arcpy.da.SearchCursor(dmap,['NAME','NUM','COUNTY','TOWN','SHAPE@X','SHAPE@Y','ELEV','RuleID','SHAPE@','BANK']) as yb :
        for row in yb:
            ...
            lc = row[1]
            xzq = row[2] + row[3]
            e = 'E' + '%0.8f'%round(F.XY2LatLon(row[4],row[5],fdh)[1],8) + '°'.decode('utf-8')
            n = 'N' + '%0.8f' % round(F.XY2LatLon(row[4],row[5],fdh)[0], 8) + '°'.decode('utf-8')
            x = '%.4f'%round(row[5],4)
            y = '%.4f'%round(row[4],4)
            h = '%.2f'%round(row[6],2)
            if row[7] == 27:
    ...
    ws.cell(1, 1, '四川省甘孜藏族自治州理塘县{}管理线桩信息表'.format(riverName.encode('utf-8')))
