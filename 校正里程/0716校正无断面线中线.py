...
import shutil
import F
path = r'E:\测试文件夹\导MDB\哈嘎'.decode('utf-8')
...
with arcpy.da.UpdateCursor(p, ['NAME', 'NUM', 'num_px', 'BANK', 'LC']) as yb:
    for row in yb:
        if row[3] == 0:
            row[0] = riverName + '-理塘县左' + '%04d' % row[2]  ###地方###
        if row[3] == 1:
            row[0] = riverName + '-理塘县右' + '%04d' % row[2]
        row[1] = 'K' + str(int(row[4] / 1000.0)) + '+' + '%03d' % (row[4]%1000)
        yb.updateRow(row)
# with arcpy.da.UpdateCursor(p, ['NAME', 'NUM', 'num_px', 'BANK', 'LC']) as yb:
#     for row in yb:
#         if row[3] == 0:
#             row[0] = riverName + '-康定市左' + '%04d' % row[2]  ###地方###
#         if row[3] == 1:
#             row[0] = riverName + '-康定市右' + '%04d' % row[2]
#         row[1] = 'K' + str(int(row[4] / 1000.0)) + '+' + '%03d' % (row[4]%1000)
#         yb.updateRow(row)
...
try:
    ...
    arcpy.CalculateField_management(p, 'RuleID', '27', 'PYTHON_9.3')
except:
    pass
shutil.rmtree(path + '\\' + 'stuff')
