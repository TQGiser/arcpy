...
for mdb in mdbs:
    ...
    if '99' in spatial_ref.name:
        with arcpy.da.SearchCursor(dmap,['SHAPE@','NAME','ELEV','RuleID','COUNTY','TOWN','NUM','RIVER']) as yb:
            for row in yb:
        ...
        with arcpy.da.SearchCursor(dmap,['SHAPE@','NAME','ELEV','RuleID','COUNTY','TOWN','NUM','RIVER']) as yb:
            for row in yb:
                if row[3] ==2 or row[3] ==3 or row[3] == 4:
                    cn = [row[0], row[1], row[2], row[3],row[4],row[5],row[6],row[7]]
                    dmaps102.append(cn)
cs99 = arcpy.da.InsertCursor(r'E:\测试文件夹\test\P99.shp',['SHAPE@','NAME','ELEV','RuleID','COUNTY','TOWN','NUM','RIVER'])
cs102 =  arcpy.da.InsertCursor(r'E:\测试文件夹\test\P102.shp',['SHAPE@','NAME','ELEV','RuleID','COUNTY','TOWN','NUM','RIVER'])
for dmap in dmaps99:
    print dmap
    cs99.insertRow(dmap)
