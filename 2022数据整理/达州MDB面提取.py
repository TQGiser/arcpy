...
df = pd.read_excel(r'E:\数据整理达州\达州4D成果目录.xlsx'.decode('utf-8'))
arcpy.env.overwriteOutput=True
path = r'E:\数据整理达州\达州4D\DLG\MDB\达州市国土空间基础信息平台数据资源体系建设项目'.decode('utf-8')
mdbPathList = []
...
for mdbPath in mdbPathList:
    print mdbPath
    P0,P1,P2,P3,GS,XMLX,XMMC,CS = mdbPath.split('\\',8)
    V = 0
    ...
    for boua in bouas:
        with arcpy.da.SearchCursor(boua, 'SHAPE@') as yb2:
            for row2 in yb2:
                cs.insertRow(row2)
    Shp = arcpy.Dissolve_management(aL, mdbPath + '\\' + 'BOUA.shp')
    arcpy.Delete_management(aL)
    arcpy.DefineProjection_management(Shp,coor_system=sf)
    Shp84 = arcpy.Project_management(Shp,mdbPath + '\\' + 'BOUA-84.shp',out_coor_system="GEOGCS['GCS_China_Geodetic_Coordinate_System_2000',DATUM['D_China_2000',"
