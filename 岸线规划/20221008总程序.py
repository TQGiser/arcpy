...
class ButtonClass37(object):
    ...
    def onClick(self):
        ...
        dmalCheckLineList = []
        dmal2CheckLineList = []
        with arcpy.da.SearchCursor(River.glq, ["SHAPE@", 'FID']) as yb1:
            for row1 in yb1:
                with arcpy.da.SearchCursor(River.dmal, "SHAPE@") as yb2:
                    for row2 in yb2:
                        if not row2[0].disjoint(row1[0]):
                            tl = row1[0].intersect(row2[0], 2)
                            if tl.partCount > 1:
                                ms ='功能区FID{}与管理范围线有{}部件'.format(row1[1], tl.partCount)
                                msList.append(ms)
                                cn = [tl, row1[1],tl.partCount]
                                dmalCheckLineList.append(cn)
                with arcpy.da.SearchCursor(River.dmal2, "SHAPE@") as yb3:
                    for row3 in yb3:
                        if not row3[0].disjoint(row1[0]):
                            tl2 = row1[0].intersect(row3[0], 2)
                            if tl2.partCount > 1:
                                ms = '功能区FID{}与2年水位线有{}部件'.format(row1[1], tl2.partCount)
                                msList.append(ms)
                                cn2 = [tl2, row1[1], tl2.partCount]
                                dmal2CheckLineList.append(cn2)
        L = arcpy.CreateFeatureclass_management(River.path, 'dmalCheckLine.shp', 'POLYLINE')
        arcpy.AddField_management(L, 'FIDA', 'SHORT')
        arcpy.AddField_management(L, 'PART', 'SHORT')
        cs = arcpy.da.InsertCursor(L, ['SHAPE@', 'FIDA','PART'])
        for line in dmalCheckLineList:
            cs.insertRow([line[0], line[1],line[2]])
        L2 = arcpy.CreateFeatureclass_management(River.path, 'dmal2CheckLine.shp', 'POLYLINE')
        arcpy.AddField_management(L2, 'FIDA', 'SHORT')
        arcpy.AddField_management(L2, 'PART', 'SHORT')
        cs2 = arcpy.da.InsertCursor(L2, ['SHAPE@', 'FIDA','PART'])
        for line in dmal2CheckLineList:
            cs2.insertRow([line[0], line[1],line[2]])
                    ...
                    for line in dmals:
                        if not line[0].disjoint(aera[0]):
                            ...
                            if m1 < m2:
                                ...
                                qd2 = p2
                                zd2 = p1
                            qd2E = XY2LatLon(qd2[0], qd2[1], fdh)[1]
                            qd2N = XY2LatLon(qd2[0], qd2[1], fdh)[0]
                            zd2E = XY2LatLon(zd2[0], zd2[1], fdh)[1]
                            zd2N = XY2LatLon(zd2[0], zd2[1], fdh)[0]
                cn2 = [0, prov, city, county, 0, ab, glq, round(lenth / 1000.0, 2), '%0.8f'%qdE, '%0.8f'%qdN, '%0.8f'%zdE, '%0.8f'%zdN, '%0.8f'%qd2E,
                       '%0.8f'%qd2N,
                       '%0.8f'%zd2E, '%0.8f'%zd2N,
                       '%0.4f'%qd[1], '%0.4f'%qd[0], '%0.4f'%zd[1],
                       '%0.4f'%zd[0], acording, id, m1, '%0.4f'%zxX, '%0.4f'%zxY, QDMAmongDmal, ZDMAmongDmal]
                groupCn.append(cn2)
...
class ButtonClass45(object):
    ...
    def onClick(self):
                ...
                if row[0] is not None:
                    ...
                    with arcpy.da.SearchCursor(dmal, 'SHAPE@') as yb4:
                        for row4 in yb4:
                            ...
                            p1x = '%0.4f'%round(row[0].firstPoint.X, 4)
                            p1y = '%0.4f'%round(row[0].firstPoint.Y, 4)
                            p2x = '%0.4f'%round(row[0].lastPoint.X, 4)
                            p2y = '%0.4f'%round(row[0].lastPoint.Y, 4)
                            if m1 < m2:
                                qdm = m1
                                zdm = m2
                                qdx = p1x
        ...
        df = pd.DataFrame(cnList2, columns=['名称', '涉河设施类别', '岸别', '县（区）', '占用岸线长度（m)', '起点坐标（Y)', '起点坐标（X)', '终点坐标（Y)', '终点坐标（X)',
                                            '起点里程', '终点里程', 'Num'])
...
#                 all = mxd.dataDrivenPages.pageCount
#                 now = mxd.dataDrivenPages.currentPageID
#                 for i in cursor:
#                     if i.FID == pagenum - 1:
#                         if i.Zbz == 0:
#                             x = TopLeftX
...
#                             x = TopRightX
#                             y = TopRightY
#                         tm = '图名:'.decode('utf-8') + str(i.Name)
#                         zbzwz = '指北针位置:'.decode('utf-8') + str(i.Zbz)
#                         jd = '进度:'.decode('utf-8') + str(now) + '/'.decode('utf-8') + str(all)
#                         print tm,zbzwz,jd
