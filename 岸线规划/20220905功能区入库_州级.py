...
try:
    arcpy.CreateFolder_management(path,'州级成果')
except:
    pass
path2 = path + '\\' + '州级成果'
glqPath = r'E:\数据\20220901\0822岸线规划\glqCGCS2000\州级'
...
for glq in glqs:
    ...
    dmal = r'E:\数据\20220901\0822岸线规划\dmalCGCS2000\{}管理范围线.shp'.format(riverName.encode('utf-8'))
    zx = r'E:\数据\20220901\0822岸线规划\中线CGCS2000\{}校正后中线.shp'.format(riverName.encode('utf-8'))
    xzqList = []
    tempShp1 = []
    bhList = []
                ...
                for row2 in yb2:
                    if row2[0].contains(row1[0]) or row2[0].overlaps(row1[0]):
                        ...
                        if BH not in bhList:
                            ...
                            # print row2[1],riverName,BH,fqmj,type,xzq_sz,xzq_qx,ab,row1[4],lng_s,lat_s,lng_e,lat_e,basis
                            tempShp1.append(cn)
    tempShps =  sorted(tempShp1,key=lambda x:x[-1], reverse=True)
    path3 = arcpy.CreateFolder_management(path2,riverName.encode('utf-8'))
