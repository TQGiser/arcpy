...
path = r'E:\2021年项目\1013岸线规划\达曲\temp'
...
for path1, name, file in aera_walk:
    for shp in file:
        if '功能区'.decode('utf-8') in shp:
            glqAera = shp
for path1, name, file in dmal_zx_walk:
    for shp in file:
        if 'DMAL' in shp:
            dmal = shp
...
cnLst = []
groupCn = []
with arcpy.da.SearchCursor(glqAera, ['SHAPE@', 'Name', 'FID', 'Acording', 'province', 'city', 'county']) as aeras:
    for aera in aeras:
        id = aera[2]
        glq = aera[1]
...
df_all.columns = ['序号', '省', '市（地）级行政区', '县级行政区', '编号', '岸别', '功能区类型', '岸线长度(km)', '起点坐标(经度）', '起点坐标(纬度）', '终点坐标(经度）',
                  '终点坐标(纬度）', 'stP_X', 'stP_Y', 'enP_X', 'enP_Y', '主要划分依据', 'FID', 'LC', '面中心X', '面中心Y']
xlsx = r'E:\2021年项目\1013岸线规划\达曲\temp'+ '\\' + '总表.xlsx'
