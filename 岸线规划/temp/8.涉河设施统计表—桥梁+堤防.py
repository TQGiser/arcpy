    ...
    for row in yb:
        ...
        with arcpy.da.SearchCursor(dmal,'SHAPE@') as yb4:
            for row4 in yb4:
                ...
                p1x = 'E' + str(round(F.XY2LatLon(row[0].firstPoint.X,row[0].firstPoint.Y,fdh)[1],8)) + '°'
                p1y ='N' + str(round(F.XY2LatLon(row[0].firstPoint.X,row[0].firstPoint.Y,fdh)[0],8))+ '°'
                p2x = 'E' + str(round(F.XY2LatLon(row[0].lastPoint.X,row[0].lastPoint.Y,fdh)[1],8)) + '°'
                p2y = 'N' + str(round(F.XY2LatLon(row[0].lastPoint.X,row[0].lastPoint.Y,fdh)[0],8)) + '°'
                if m1<m2:
                    qdm = m1
                    zdm = m2
                    qdx = p1x
...
try:
    ...
    print '没有堤防'
cnList2 = sorted(cnList,key = lambda s:s[10])
