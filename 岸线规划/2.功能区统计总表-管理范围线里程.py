...
with arcpy.da.SearchCursor(glqAera, ['SHAPE@', 'Name', 'FID', 'According', 'province', 'city', 'county']) as aeras:
    for aera in aeras:
        ...
        with arcpy.da.SearchCursor(dmal, 'SHAPE@') as dmals:
            for line in dmals:
                if not line[0].disjoint(aera[0]):
                    ...
                    # if ab == '左岸':
                    #     QDMAmongDmal = 'LK' + str(int(line[0].measureOnLine(qdp) / 1000.0)) + '+' + '%03d' % (
                    #                 line[0].measureOnLine(qdp) % 1000)
                    #     ZDMAmongDmal = 'LK' + str(int(line[0].measureOnLine(zdp) / 1000.0)) + '+' + '%03d' % (
                    #                 line[0].measureOnLine(zdp) % 1000)
                    # elif ab == '右岸':
                    #     QDMAmongDmal = 'RK' + str(int(line[0].measureOnLine(qdp) / 1000.0)) + '+' + '%03d' % (
                    #             line[0].measureOnLine(qdp) % 1000)
                    #     ZDMAmongDmal = 'RK' + str(int(line[0].measureOnLine(zdp) / 1000.0)) + '+' + '%03d' % (
                    #             line[0].measureOnLine(zdp) % 1000)
        ...
        cn2 = [0, prov, city, county, 0, ab, glq, round(lenth / 1000.0, 2), qdE, qdN, zdE, zdN, qd2E, qd2N, zd2E, zd2N,
               qd[1], qd[0], zd[1],
...
df2 = df.groupby('ab')
df_all = pd.DataFrame(
    columns=['num', 'pro', 'city', 'county', 'bh', 'ab', 'glq', 'lenth', 'ste', 'stn', 'ene', 'enn', 'ste2', 'stn2',
             'ene2', 'enn2', 'stx', 'sty',
...
xlsx = path + '\\' + '{}总表.xlsx'.format(riverName.encode('utf-8'))
