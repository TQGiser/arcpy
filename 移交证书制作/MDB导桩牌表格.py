        ...
        for row in yb:
            if row[6] ==2 or row[6] ==3 or row[6] == 4:
                ...
                if row[6] == 2 or row[6] == 3:
                    ...
                    bt = '告示牌'
                tk2K = F.listFile(r'E:\康定划界资料\康定图框','康定市2000图框成果'.decode('utf-8'),'m')
                tk1W = F.listFile(r'E:\康定划界资料\康定图框','康定市10000图框成果'.decode('utf-8'),'m')
                tfname = []
...
df = pd.DataFrame(cnList,columns=['桩名','里程','位置','经度','纬度','X','Y','高程','表头','图幅','编号','辖区','地名',
                                  '单位','日期','river','备注'])
df.to_excel(r'E:\测试文件夹\康定数据库\a.xlsx'.decode('utf-8'),index=False)
