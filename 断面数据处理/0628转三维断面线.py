...
import math
import pinyin
path = r'E:\测试文件夹\断面线制作\temp'.decode('utf-8')
...
dmxJd = arcpy.Intersect_analysis(in_features= '{} #;{} #'.format(dmx.encode('utf-8'),JzhZx),out_feature_class=dmLcd,output_type='POINT')
jdTable = path + '\\' + 'stuff' + '\\' + 'jdTable_test'
arcpy.LocateFeaturesAlongRoutes_lr(in_features=dmxJd,in_routes=JzhZx, route_id_field="NAME", radius_or_tolerance="10 Meters", out_table=jdTable
                                   , out_event_properties="RID POINT MEAS", route_locations="FIRST", distance_field="DISTANCE", zero_length_events="ZERO",
                                   in_fields="NO_FIELDS", m_direction_offsetting="M_DIRECTON")                          #NO_FIELDS 保证乱码字段不输出
arcpy.JoinField_management(in_data=dmx,in_field="Lj", join_table=jdTable, join_field="INPUTOID", fields="MEAS")
dmx_sorted = path + '\\' + 'dmx_OK.shp'
...
with arcpy.da.UpdateCursor(dmx_sorted,['dmh','MEAS','里程','间距']) as yb:
    ...
    for row in yb:
        ...
        row[2] = 'K' + str(int(row[1]/1000)).split('.')[0] + '+' + str('%0.3f'%round(row[1]/1000,3)).split('.')[1][0:3]
        row[3] = abs(int(str(int(row[1]/1000)).split('.')[0] + str('%0.3f'%round(row[1]/1000,3)).split('.')[1][0:3])-lcList[-1])
        lcList.append(int(str(int(row[1]/1000)).split('.')[0] + str('%0.3f'%round(row[1]/1000,3)).split('.')[1][0:3]))
        yb.updateRow(row)
        a +=1
