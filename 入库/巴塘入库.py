...
df = pd.read_excel(r'E:\2021年项目\1126巴塘改\巴塘县38条河流统计表（所有河流名称和长度以此为准）.xlsx'.decode('utf-8'))
path = r'E:\2021年项目\1115错纳柯\shape'
...
for dmap in dmaps:
    ...
    try:
        ...
        pass
    mdb = arcpy.Copy_management(r'E:\2021年项目\1126巴塘改\模板.mdb',r'E:\测试文件夹\导MDB\{}.mdb'.format(riverName))
    ...
    with arcpy.da.UpdateCursor(dmaaInMdb, ['GB', 'NAME','DATE_', 'VERS', 'RuleID', 'RIVER']) as yb:
        for row in yb:
            row[0] = 910100
            row[1] = '巴塘县{}河道管理范围'.format(riverName)                                                                              #########
            row[2] = '202111'
            row[3] = '2021'
            row[4] = 3
            row[5] = riverName
