...
class ButtonClass37(object):
    ...
    def onClick(self):
        ...
        arcpy.CreateFileGDB_management(River.path, 'check.gdb')
        arcpy.env.workspace = River.path + '\\' + 'check.gdb'
        tl1 = arcpy.Clip_analysis(River.dmal, River.glq)
        tl2 = arcpy.SymDiff_analysis(tl1, River.dmal, '外边错误线')
        tl3 = arcpy.Clip_analysis(River.dmal2, River.glq)
        tl4 = arcpy.SymDiff_analysis(tl3, River.dmal2, '内边错误线')
        arcpy.Delete_management(tl1)
        arcpy.Delete_management(tl3)
        if math.isnan(arcpy.Describe(tl2).extent.XMin) is not True:
            ms = '管理范围线有错误'
            msList.append(ms)
        if math.isnan(arcpy.Describe(tl4).extent.XMin) is not True:
            ms = '2年水位线有错误'
            msList.append(ms)
