...
import re
DLMC = '雅江县段'
path = r'D:\2021年项目\0720雅江图册\影像\分幅\分幅'.decode('utf-8')
...
for ras in arcpy.ListRasters('*','tif'):
    ...
    rasResampled = ras.split('.tif')[0] + '-resample.tif'
    arcpy.Resample_management(ras,rasResampled,'0.5 0.5')
    arcpy.env.compression = 'NONE'
    arcpy.CopyRaster_management(rasResampled,rasProcessed,pixel_type='8_BIT_UNSIGNED',format='TIFF')
    arcpy.Delete_management(ras)
    arcpy.DefineProjection_management(rasProcessed,coor_system='4543')
print('影像预处理完成')
arcpy.env.workspace = data_path
tmpShp = r'fw.shp'
tmpShp2 = r'甘孜1W图框102投影.shp'
...
with arcpy.da.UpdateCursor(joinShpSorted,['图幅编号','TK','name']) as yb:
    for row in yb:
        row[0] = row[2] + DLMC.decode('utf-8') + '-' + row[1]
        yb.updateRow(row)
...
for i,v,m in os.walk(u_data_path):
    for name6 in m:
        if name6.endswith('.tif') and 'resample' not in name6:
            tiflis.append(name6)
    for name7 in m:
        if name7.endswith('.tfw') and 'resample' not in name6:
            tfwlis.append(name7)
for file2 in tfwlis:
